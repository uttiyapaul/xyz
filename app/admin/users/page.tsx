...
export default async function ManageUsersPage() {
    ...
    // Fetch lists for the Verification Modal
    const [orgsRes, rolesRes] = await Promise.all([
        supabaseAdmin.from("client_organizations").select("id, legal_name").is("deleted_at", null).order("legal_name"),
    ...
    return (
        <div style={{ padding: "32px", color: "#FAFAF8" }}>
            ...
            {error ? (
                <div style={{ color: "#FF3B30", background: "rgba(255, 59, 48, 0.1)", padding: "16px", borderRadius: "8px" }}>
                    {error}
                </div>
            ) : (
