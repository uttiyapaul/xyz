...
export default function SourcesPage() {
  ...
  async function loadData(activeOrgId: string) {
    ...
    try {
      const [{ data: siteData }, { data: sourceData }] = await Promise.all([
        supabase
          .from("v_active_sites")
      ...
      const nextSources = ((sourceData ?? []) as Omit<EmissionSource, "site_name">[]).map((source) => ({
        ...source,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await supabase.from("ghg_emission_source_register").insert({
        organization_id: orgId,
        site_id: form.site_id || null,
        fy_year: getCurrentFyYearLabel(),
        ...
        <div>
          <div style={{ background: "#0D0D14", border: "1px solid #1A1A24", borderRadius: "6px", padding: "20px" }}>
            ...
            <p style={{ fontSize: "12px", color: "#6B7280", marginTop: 0, marginBottom: "16px" }}>
              Entries now write to the live GHG source register for {getCurrentFyYearLabel()}.
            </p>
                  ...
                  <div key={scope} style={{ marginBottom: "24px" }}>
                    ...
                    <div style={{ display: "grid", gap: "8px" }}>
                      {scopeSources.map((source) => (
                        <div key={source.id} style={{ background: "#07070E", border: "1px solid #1A1A24", borderRadius: "4px", padding: "12px" }}>
                          <div style={{ fontSize: "14px", color: "#FAFAF8", marginBottom: "4px" }}>{source.source_name}</div>
